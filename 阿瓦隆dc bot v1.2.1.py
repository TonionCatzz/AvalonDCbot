...
async def room(ctx, room_number: str):
    ...
    success, driver, msg = join_room(room_number)
    await ctx.send(msg)
...
async def record(ctx):
    ...
    try:
        ...
        # 執行你的處理邏輯
        success, msg_players_role, msg_players_list, msg_players_ticket, msg_kill ,msg_tag = record_after_game(driver, user_input.content)
...
async def help(ctx):
    embed = Embed(
        ...
        description="這是用來輔助 [Avalon 遊戲網站](https://avalon.signage-cloud.org/) 的工具，以下是可用指令：",
        color=0x3498db  # 藍色
    )
